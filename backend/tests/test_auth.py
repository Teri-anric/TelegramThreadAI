...
def test_telegram_login_success(db_session: Session, login_data: dict):
    ...
    with patch('app.db.repos.user.UserRepository.create_or_update_user') as mock_create_or_update_user:
        mock_create_or_update_user.return_value = User(
            id=61289,
            telegram_id=login_data["credentials"]["id"]
        )
        print(login_data)
...
@pytest.mark.parametrize("status_code, login_data", [
    (400, {
        "login_type": "tg_login_widget",
        "credentials": {
            "id": "123456789",
...
def test_telegram_login_failure(db_session: Session, status_code: int, login_data: dict):
    """Test Telegram login endpoint."""
    ...
    assert response.status_code == status_code, f"Login failed with status code {response.status_code}"
